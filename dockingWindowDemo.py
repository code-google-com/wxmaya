...
class PyAUIFrame(wx.Frame):
    ...
    def CreateTreeCtrl(self):
        ...
        root = tree.AddRoot("Maya Dag Nodes")
        items = []
        ...
        if isMayaRunning:
            for each in m.ls(dag=True, transforms=True):
                ...
                for children in m.listRelatives(each):
                    tree.AppendItem(id, children, 0)
        else:
            tree.AppendItem(root, 'Maya Not Running!', 0)
...
<html><body>
<h4>
This demo is a wxmaya version of the AUI_DockingWindowMgr found in the standard wxPython Demo app.<br><br>
